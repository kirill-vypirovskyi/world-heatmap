const BINARY_DIMENSION_X = 36e3;
const DIMENSION_Y = 17999;
const canvas = document.createElement("canvas");
const context = canvas.getContext("2d");
const backgroundImage = new Image();
backgroundImage.src = "empty-map.jpg";
...
const createHeatMap = (file) => new Promise((resolve, reject) => {
  ...
  reader.onload = () => {
    ...
    let counterX = 0;
    let counterY = 0;
    for (let y = DIMENSION_Y - 1; y >= 0; y -= Math.ceil(DIMENSION_Y / canvas.height)) {
      for (let x = 0; x < BINARY_DIMENSION_X; x += Math.ceil(BINARY_DIMENSION_X / canvas.width)) {
